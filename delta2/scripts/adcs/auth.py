...
class Authenticate:
    ...
    def authenticate(
        ...
        if not is_key_credential:
            ...
            if len(identifications) > 1:
                print("Found multiple identifications in certificate")
        ...
        if not any([len(username), len(domain)]):
            raise Exception("Username or domain is invalid: %s@%s" % (username, domain))
            return False
    ...
    def ldap_authentication(
        ...
        print("Connecting to %s" % repr("%s://%s:%d" % (self.ldap_scheme, host, self.ldap_port)))
        ...
        try:
            ...
            raise Exception("LDAP not configured for SSL/TLS connections")
            if self.verbose:
                raise e
            return False
    ...
    def kerberos_authentication(
        ...
        try:
            ...
            if "KDC_ERR_CLIENT_NAME_MISMATCH" in str(e) and not is_key_credential:
                ...
                raise Exception(("Wrong domain name specified %s" % repr(domain)))
                if id_type is not None:
                    raise Exception(
                ...
                raise Exception("Got error while trying to request TGT: %s" % str(e))
        ...
        for pa in as_rep["padata"]:
            ...
            raise Exception("PA_PK_AS_REP was not found in AS_REP")
            return False
        ...
        if key_info["content_type"] != "1.3.6.1.5.2.3.2":
            raise Exception("Unexpected value for key info content type")
            return False
        ...
        if etype == Enctype.AES256:
            ...
            raise Exception("Unexpected encryption type in AS_REP")
            return False
        ...
        if not self.no_save or self.ptt:
            if not self.no_save:
                if self.kirbi:
                    ...
                    print("Saved Kirbi file to %s" % repr(kirbi_name))
                else:
            ...
            if self.ptt:
                krb_cred = ccache.toKRBCRED()
                print("Trying to inject ticket into session")

                if platform.system().lower() != "windows":
                    raise Exception("Not running on Windows platform. Aborting")
                else:
                    try:
                        ...
                        if res:
                            print("Successfully injected ticket into session")
        ...
        if not self.no_hash:
            print("Trying to retrieve NT hash for %s" % repr(username))
            ...
            if not is_key_credential:
                print("Got hash for %s: %s:%s", repr(upn), lm_hash, nt_hash)
