# From certipy github: https://github.com/ly4k/Certipy/blob/main/certipy/commands/find.py modified by: Ceald
import copy, json, os, socket, struct, time, zipfile
...
class Find:
    def __init__(self, target: Target, connection: Connection,
                json:bool = False, text: bool = False, stdout:bool = False,
                output:str = None, enabled: bool = False, vulnerable:bool = False,
                hide_admins: bool = False, dc_only: bool = False, scheme: str = "ldaps",
                ):
    ...
    def open_remote_registry(self, target_ip: str, dns_host_name: str):
        ...
        for _ in range(3):
            ...
            print("Failed to connect to remote registry")
            return None
        ...
        for ca in cas:
            ...
            for ca in cas:
                if self.dc_only:
                    ...
                    try:
                        ca_name = ca.get("name")
                        ca_remote_name = ca.get("dNHostName")
    ...
    def check_web_enrollment(self, ca: LDAPEntry) -> bool:
        ...
        try:
            ...
            print("Connecting to %s:80" % target_ip)
